...
if(mysql_num_rows($run)>0){
	...
	$query=mysql_query(" insert into proposal(sender_email,reciver_email,event_id, special,proposal,proposal_date)values('$_SESSION[email]' , '$email','$id','$special','$about','$date' )")or die('error in fallow  insertion query');
 if($query){
...
$notification=$name[0]." ".$name[1]." send you proposal ";

$q1=mysql_query("INSERT INTO `ema`.`notification` (`from_email`, `to_email`,`event_id`, `notification`) VALUES ('$_SESSION[email]','$email','$id', '$notification');")or die('error in  notification insertion query');
