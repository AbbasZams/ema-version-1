				...
				<header>
					 ...
					 <?php
                            ...
                            mysql_select_db("ema",$con);
								$profilepicture = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$email' and `for`='profile'")or die('profile picture selection  query error');
...
?>
										...
										<div class="col-sm-12">
											...
											?>
												...
												<div class="comp-brand img-responsive">
													<img src="profile_pictures/<?php echo $row[0];?>" class="img-responsive">
												</div>
												<?php
											}
												else{
								...
								<div class="col-md-8">
											...
											<table align="center" class="table table-hover">
												<tbody>
													...
													<tr  align="center">
														...
														<td>Charge</td>
														<td>Rate</td>
														<td><strong style="font-weight: bolder;">Select</strong></td>
													</tr>
													<?php
									...
									<div class="row">
										<div class="col-md-10 col-md-offset-1 form-group">
											<div class="col-md-12">
												<button class="btn btn-warning" style="float: right; font-size: 14px; width: 200px; letter-spacing: 2px; padding: 6px;" type="submit" name="order">Send Order</button>
...
if($chk=="" or $eventrequirement=="" ){
	...
	$ordercount=mysql_query("SELECT COUNT(order_no) from ema.order") or die('order count selection query error');
				 $ordernumber=mysql_fetch_array($ordercount);
				 $orderno=$ordernumber[0];
				$orderno=$orderno+1;
		$q=mysql_query("INSERT INTO `order`(`sender_email`,`organizer_email`,`event_id`, `order_note`,`order_no`) VALUES('$_SESSION[email]','$email','$id','$eventrequirement','$orderno' )");
		if($q){
...
$con=mysql_connect("localhost","root", "") or die("database connectivity error");
	...
	$ordercount=mysql_query("SELECT COUNT(order_no)  from ema.order") or die('order count selection query error');
				$ordernumber=mysql_fetch_array($ordercount);
				  $orderno=$ordernumber[0];
				$orderno=$orderno+1;
		$qe=mysql_query("INSERT INTO `order`(`sender_email`,`organizer_email`,`event_id`, `order_note`,`order_no`) VALUES('$_SESSION[email]','$email','$id','$orderdetails','$orderno' )");
}
