...
.organizer-row{
 ...
 $db=mysql_select_db('ema',$con) or die('database selection  query error');
   $query=mysql_query(" select user.fristname,user.lastname,organizer.organizer_email,organizer.companyname,organizer.country,organizer.city,organizer.special from user JOIN organizer where user.email=organizer.organizer_email") or die('organizer   table selection query error');
      ...
      <div class="col-md-4" style="padding-top: 20px;padding-bottom: 20px;">
        <div class="organizer-div" align="center">
            ...
            <?php
                ...
                $img = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$rows[2]' and `for`='profile'")or die('organizar picture selection  query error');
                if(mysql_num_rows($img)>0){
                                 $orgimg=mysql_fetch_array($img)
                ?>

              <img src="profile_pictures/<?php echo $orgimg[0];?>" class="img-circle">
              <?php
            }
            else{
              ?>
              <img src="" class="img-circle" alt="No profile picture">
              <?php
            }
            ?>
