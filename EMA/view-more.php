	...
	<body>
		...
		<!-- Header -->
			<section id="header">
				...
				<nav id="nav">
					<ul>
						<li><a href="#one" class="active">Photo gallery</a></li>
						<li><a href="#two">Videos</a></li>
  ...
  if(mysql_num_rows($result)>0){
	   $rows=mysql_fetch_array($result); 
		  ?>
					...
					<br>
					<h3 style="color:#286E88; "> &nbsp; Photo gallery:</h3>
					<div class="row" >
					<?php
					$query = "  SELECT `name`,`serial_no` FROM `event_picture` WHERE event_id='$id'" ;

$run = mysql_query($query)or die('selection query error');
...
?>
						...
						<div class="col-md-4" align="center">
							<div  >
								<img src="event_picture/<?php echo $name;?>" style="height:200px;width:200px;padding:10px;"  class="img-responsive" >
							</div>
							<?php
 if($email==$_SESSION['email']){
 	?>

								 <form method="post" name="deletepic" action="">
 		<input type="hidden" name="serial" value="<?php echo $row[1];?>">
						<button name="deletepic" type="submit" class="btn btn-danger" onclick="return window.confirm('are you sure to delete?')" style="margin-left: 45%;"  title="Delete this Picture">Delete</button>
						</form>	
						<?php
					}
					?>
...
?>
	  ...
	  <center>There is no any picture of this event.ok</center>
	  </font>
	  </p>
	  <?php	
}
?>					
					...
					<section id="two" >
					<div class="container-fluid">
					<h3 style="color:#286E88;"> Videos:</h3>
					
						<div class="row ">

						
						<?php
					$query1 = "  SELECT `name`,`serial_no` FROM `event_video` WHERE event_id='$id'" ;

$run1 = mysql_query($query1)or die('selection query error');
...
?>
						...
						<div class="col-md-4 img-responsive" align="center">
							 <video controls   style="height:200px;width:200px;padding:10px;">
<source src="event_video/<?php echo $video; ?>">
 </video> <br>
 <?php
 if($email==$_SESSION['email']){
 	?>
 		<form method="post" name="deletevideo" action="">
 		<input type="hidden" name="serialno" value="<?php echo $row[1];?>">
						<button name="deletevideo" type="submit" class="btn btn-danger" onclick="return window.confirm('are you sure to delete?')" style="margin-left: 45%;"  title="Delete this video">Delete</button>
						</form>	
						<?php
					}
					?>
...
?>
	  <p><font color="green" size="5">
	  <center>There is no any video of this event.ok</center>
	  </font>
	  </p>
	  <?php	
}
...
if(isset($_POST['deletevideo'])){
	$videoid=$_POST['serialno'];

	$q=mysql_query(" DELETE FROM `ema`.`event_video` WHERE `event_video`.`serial_no`='$videoid'") Or die("error in video  delete  query");
	if($q){
		echo "<script>alert('video deleded successfully')</script>";
		?>
		<meta http-equiv="refresh" content="0">
		<?php
	}
	else{
		echo "<script>alert(' video deletion failed! ')</script>";
	}
}
if(isset($_POST['deletepic'])){
	$picid=$_POST['serial'];

	$q=mysql_query(" DELETE FROM `ema`.`event_picture` WHERE `event_picture`.`serial_no`='$picid'") Or die("error in pic  delete  query");
	if($q){
		echo "<script>alert('picture deleded successfully')</script>";
		?>
		<meta http-equiv="refresh" content="0">
		<?php
	}
	else{
		echo "<script>alert('picture deletion failed! ')</script>";
	}
}
?>