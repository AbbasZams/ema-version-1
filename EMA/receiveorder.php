      ...
      <section id="header">
        <header>
                            ...
                            mysql_select_db("ema",$con);
								$profilepicture = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$_SESSION[email]' and `for`='logo'")or die('logo selection  query error');
                ...
                <img src="profile_pictures/<?php echo $row[0];?>" class="img-responsive" alt="" />
					...
					<?php
									...
									<?php
									 ...
									 $username = mysql_query(" SELECT `companyname` FROM `organizer` WHERE organizer_email='$_SESSION[email]' ")or die('company selection  query error');
...
if(mysql_num_rows($query)>0){
  while($emailrows=mysql_fetch_array($query)){
    $senderemail=$emailrows[0];
     $ordersender=mysql_query(" select user.fristname,user.lastname  from user  where  user.email='$senderemail'") or die('order sender table selection query error');
          ...
          <div class="row">
            <div class="col-md-3" align="center">
               ...
               <?php
                ...
                $img = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$senderemail' and `for`='profile'")or die('organizar picture selection  query error');
                if(mysql_num_rows($img)>0){
                                 $orgimg=mysql_fetch_array($img)
                ?>
                <img src="profile_pictures/<?php echo $orgimg[0];?>" class="img-rounded">
                <?php
            }
            else{
              ?>
              <img src=""  class="img-rounded" alt="No profile picture">
              <?php
            }
            ?>
            </div>
...
$sender_email=$email_rows[0];
     $order_sender=mysql_query("select user.fristname,user.lastname  from user  where  user.email='$sender_email'") or die(' read order sender table selection query error');
          ...
          <div class="row">
            <div class="col-md-3" align="center">
               <div class="organizer-image">
                ...
                $imgs = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$sender_email' and `for`='profile'")or die('organizar picture selection  query error');
                if(mysql_num_rows($imgs)>0){
                                 $orgimgs=mysql_fetch_array($imgs)
                ?>
                <img src="profile_pictures/<?php echo $orgimgs[0];?>" class="img-rounded">
                <?php
            }
            else{
              ?>
...
$sender_emails=$email_rowss[0];
     $order_senders=mysql_query("select user.fristname,user.lastname  from user  where  user.email='$sender_emails'") or die(' read order sender table selection query error');
          ...
          <div class="row">
            <div class="col-md-3" align="center">
               <div class="organizer-image">
                ...
                $imgss = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$sender_emails' and `for`='profile'")or die('organizar picture selection  query error');
                if(mysql_num_rows($imgss)>0){
                                 $orgimgss=mysql_fetch_array($imgss)
                ?>
                <img src="profile_pictures/<?php echo $orgimgss[0];?>" class="img-rounded">
                <?php
            }
            else{
              ?>
            ...
            <div style="padding: 5px;">
              ...
              <?php
              $pay=mysql_query("SELECT * from `ema`.`order_payment` where `sender_email`='$email_rowss[0]'  and `reciver_email`='$email_rowss[1]' and `order_no`='$email_rowss[4]' and `evant_id`='$email_rowss[2]' ") or die("payment selection query error");
...
$sender_emailc=$email_rowsc[0];
     $order_senderc=mysql_query("select user.fristname,user.lastname  from user  where  user.email='$sender_emailc'") or die(' read order sender table selection query error');
          ...
          <div class="row">
            <div class="col-md-3" align="center">
               <div class="organizer-image">
                ...
                $imgsc = mysql_query("SELECT `name` FROM `profile_picture` WHERE email='$sender_emailc' and `for`='profile'")or die('completed picture selection  query error');
                if(mysql_num_rows($imgsc)>0){
                                 $orgimgsc=mysql_fetch_array($imgsc)
                ?>
                <img src="profile_pictures/<?php echo $orgimgsc[0];?>" class="img-rounded">
                <?php
            }
            else{
              ?>
...
<?php         
             ...
             <button type="submit"  name="viewall" class="btn btn-success allorder" style="margin-left: 45%; letter-spacing: 2px;" >VIEW ALL ORDERS</button>
