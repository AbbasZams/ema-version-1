...
$db = mysql_select_db('ema',$con)or die('database selection error');
		 $date= Date('20y-m-d');
	$insert=mysql_query(" INSERT INTO `ema`.`user` (`fristname`, `lastname`, `mobilenumber`, `email`, `password`, `gender`, `dateofbrith`, `joindate`) VALUES ('$_SESSION[fristname]', '$_SESSION[lastname]', '$_SESSION[mobilenumber]', '$_SESSION[email]', '$_SESSION[password]', '$_SESSION[gender]', '$_SESSION[dob]', '$date')") or die('insertion query error');
		if($insert){
