									...
									<div class="well">
										<div class="row">
											<div class="col-md-3" >
												<div align="center">
												<?php
	  $picture=mysql_query("SELECT `name` FROM ema.`profile_picture` WHERE email='$email' and `for`='profile'") or die(' reviewer picture selection querr error');
	  if(mysql_num_rows($picture)>0){
		  $orgphoto=mysql_fetch_array($picture)
		  ?>
													<div style="width: 140px; height: 120px;  " >
														<img src="profile_pictures/<?php echo $orgphoto[0];?>" style="width: 140px; height: 120px;  " class="img-responsive img-rounded">
													</div>
													<?php
										}else{
											?>
											<div style="width: 140px; height: 120px; background-color: red; float: right;">
										...
										?>
													<div>
													<?php
												$user=mysql_query("SELECT `fristname`,`lastname` FROM `user` WHERE email='$email'") or die(' user name selection querr error');
									...
									?>
									<div class="well">
										<h2 align="center"> NO proposal</h2>
									</div>
									<?php
								}
								?>
